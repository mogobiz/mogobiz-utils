...
trait HttpRequestor {
  ...
  val badSslCtx = Http().createClientHttpsContext(badSslConfig)
  ...
  def doRequest(request: HttpRequest): Future[HttpResponse] = {
    ...
    val port =
      if (uri.authority.port != 0)
        ...
        80
    if (isHttps) {
      val connectionFlow = Http().outgoingConnectionHttps(host = uri.authority.host.address(), port = port, connectionContext = badSslCtx)
